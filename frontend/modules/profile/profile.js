...
async function loadProfileData() {
  try {
    const res = await fetch(`${PROFILE_API}/me`, { headers: getAuthHeaders() });
...
async function profileSavePreferences() {
  ...
  try {
    const res = await fetch(`${PROFILE_API}/preferences`, {
      method: "PUT",
      headers: getAuthHeaders(),
...
function checkAndShowWeeklyDigest() {
  ...
  setTimeout(async () => {
    let statsLine = "";
    try {
      const res = await fetch("http://127.0.0.1:5050/api/kpi", { headers: getAuthHeaders() });
      if (res.ok) {
